...
from datasets import MetaImageModuloDataset
...
def meta_train(
    ...
    for epoch in range(epochs):
        ...
        for _ in range(tasks_per_meta_batch):
            # Sample a task
            X_s, _, y_s, X_q, _, y_q, _ = dataset.sample_task()
            X_s, y_s, X_q, y_q = (
                X_s.to(device),
                y_s.to(device),
                X_q.to(device),
...
def meta_test(meta, dataset, criterion, device, n_tasks, adaptation_steps):
    ...
    for t in range(n_tasks):
        X_s, _, y_s, X_q, X_num_q, y_q, m = dataset.tasks[t]
...
def main(
    ...
    # init datset
    dataset = MetaImageModuloDataset(n_tasks, n_samples_per_task, range_max=100)
